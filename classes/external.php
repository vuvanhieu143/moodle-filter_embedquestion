...
class external extends \core_external\external_api {
     ...
     */
    public static function get_sharable_question_choices_parameters(): \core_external\external_function_parameters {
        return new \core_external\external_function_parameters([
            'cmid' => new \core_external\external_value(PARAM_INT, 'Course module ID'),
            'categoryidnumber' => new \core_external\external_value(PARAM_RAW, 'Idnumber of the question category.'),
        ]);
    }
     ...
     *
     * @return \core_external\external_description Result type
     */
    public static function get_sharable_question_choices_returns(): \core_external\external_description {
        return new \core_external\external_multiple_structure(
            new \core_external\external_single_structure([
                'value' => new \core_external\external_value(PARAM_RAW, 'Choice value to return from the form.'),
                'label' => new \core_external\external_value(PARAM_RAW, 'Choice name, to display to users.'),
            ])
        );
    }
     ...
     * @return \core_external\external_function_parameters Parameters
     */
    public static function get_embed_code_parameters(): \core_external\external_function_parameters {
        // We can't use things like PARAM_INT for things like variant, because it is
        // and int of '' for not set.
        return new \core_external\external_function_parameters([
            'cmid' => new \core_external\external_value(
                PARAM_INT,
                'Course module id of the question bank.'
            ),
            'categoryidnumber' => new \core_external\external_value(
                PARAM_RAW,
                'Id number of the question category.'
            ),
            'questionidnumber' => new \core_external\external_value(
                PARAM_RAW,
                'Id number of the question.'
            ),
            'iframedescription' => new \core_external\external_value(
                PARAM_TEXT,
                'Iframe description.'
            ),
            'behaviour' => new \core_external\external_value(
                PARAM_RAW,
                'Question behaviour.'
            ),
            'maxmark' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Question maximum mark (float or "").'
            ),
            'variant' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Question variant (int or "").'
            ),
            'correctness' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Whether to show question correctness (1/0/"") for show, hide or default.'
            ),
            'marks' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Wheter to show mark information (0/1/2/"") for hide, show max only, show mark and max or default.'
            ),
            'markdp' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Decimal places to use when outputting grades.'
            ),
            'feedback' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Whether to show specific feedback (1/0/"") for show, hide or default.'
            ),
            'generalfeedback' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Whether to show general feedback (1/0/"") for show, hide or default.'
            ),
            'rightanswer' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Whether to show the automatically generated right answer display (1/0/"") for show, hide or default.'
            ),
            'history' => new \core_external\external_value(
                PARAM_RAW_TRIMMED,
                'Whether to show the response history (1/0/"") for show, hide or default.'
            ),
            'forcedlanguage' => new \core_external\external_value(
                PARAM_LANG,
    ...
    /**
     * Returns result type for the get_embed_code function.
     *
     * @return \core_external\external_description Result type
     */
    public static function get_embed_code_returns(): \core_external\external_description {
        return new \core_external\external_value(
            PARAM_RAW,
